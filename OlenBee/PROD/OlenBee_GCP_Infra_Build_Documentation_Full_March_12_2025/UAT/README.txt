...
At the top level, the Olenbee.com organization acts as the root entity, encapsulating all projects and resources under a unified management structure.
 ...
 Network
 .......
 Manages network configurations for application environments, including PRD, DEV, PPRD, UAT, and UAT.
 ...
 Note: All projects use a shared VPC comming from olb-nethub-<env>, being <env> each environment (prd,dev, uat, UAT)
 ...
 Contains critical application components:
  - Beneficiaries-API (e.g., olb-beneficiaries-api-UAT)
...
...........................................
 ...
 - Network – Defines networking configurations for different environments (PRD, DEV, PPRD, UAT, UAT).  
   Note: All projects use a shared VPC comming from olb-nethub-<env>, being <env> each environment (prd,dev, uat, UAT)
       The app projects need to be related inside VPC --> Shared VPC section in order to be visible and usable on the destination app project.
 ...
 - Production (PRD) – Hosts production applications, including backend and frontend services.
  ...
  └── STANDARD <-- Parent Folder
      ...
      |
      ├── Network  ---> Network configurations for App Projects (PRD - DEV - PPRD - UAT - UAT)
      |   |_ Note: All projects use a shared VPC comming from olb-nethub-<env>, being <env> each environment (prd,dev, uat, UAT)
      |   |        The app projects need to be related inside VPC --> Shared VPC section in order to be visible and usable on the destination app project.
      |   |
      │   ├── olb-nethub-prd  (olb-nethub-prd-e7ce) 
      │   ├── olb-netspoke-uat (olb-netspoke-uat-6005) 
      │   ├── olb-netspoke-pprd (olb-netspoke-pprd-b534) 
      │   ├── olb-netspoke-prd (olb-netspoke-prd-39fa)  
      │   ├── olb-netspoke-UAT (olb-netspoke-UAT-4b74)
      │   └── olb-netspoke-uat (olb-netspoke-uat-8f33) 
      |
      ├── Security  ---> Security Configuirations for App Projects (PRD - DEV - PPRD - UAT - UAT)
      |   |
      ...
      └── UAT  ---> UAT APPS
          ...
          ├── Beneficiaries-API 
          |   | 
          │   └── olb-beneficiaries-api-uat  
          |
          ├── Clients-WEBAPP
          |   |  
          │   ├── Backend  
          │   │   └── olb-clientsapp-api-uat  
          │   └── Frontend  
          │       └── olb-clientsapp-front-uat
          | 
          └── Olenbee-BO  
              |
              ├── Backend  
              │   └── olb-backoffice-api-uat
              └── Frontend  
                  └── olb-backoffice-front-uat 
...
1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-clientsapp-api-uat/clientsapp/clientsapp .
   - docker psuh europe-west9-docker.pkg.dev/olb-clientsapp-api-uat/clientsapp/clientsapp:latest

2- Deploy the Cloud Run instance

gcloud run deploy clienstapp \
    --image europe-west9-docker.pkg.dev/olb-clientsapp-api-uat/clientsapp/clientsapp:latest \
    --region europe-west9 \
    --platform managed \
    --service-account clourun-olenbee@olb-clientsapp-api-uat.iam.gserviceaccount.com \
    --vpc-connector projects/olb-clientsapp-api-uat/locations/europe-west9/connectors/olb-uat-network-glb-netspoke \
    --vpc-egress all-traffic \
    --port 8080 \
    --allow-unauthenticated
...
ClientsApp Front UAT:
.....................
This section outlines the steps to build and deploy the frontend application for ClientsApp in a dev environment.

1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-clientsapp-front-uat/clientsapp-front/clientsapp-front .
   - docker psuh europe-west9-docker.pkg.dev/olb-clientsapp-front-uat/clientsapp-front/clientsapp-front:latest

2- Deploy the Cloud Run instance

gcloud run deploy clientsapp-front \
   --image europe-west9-docker.pkg.dev/olb-clientsapp-front-uat/clientsapp-front/clientsapp-front:latest \
   --region europe-west9 \
   --platform managed \
   --service-account clourun-olenbee@olb-clientsapp-front-uat.iam.gserviceaccount.com \
   --vpc-connector projects/olb-clientsapp-api-uat/locations/europe-west9/connectors/olb-uat-network-glb-netspoke \
   --vpc-egress all-traffic \
   --port 8080 \
   --allow-unauthenticated
...
1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-backoffice-api-uat/backoffice/backoffice .
   - docker psuh europe-west9-docker.pkg.dev/olb-backoffice-api-uat/backoffice/backoffice:latest

2- Deploy the Cloud Run instance

gcloud run deploy backoffice \
    --image europe-west9-docker.pkg.dev/olb-backoffice-api-uat/backoffice/backoffice:latest \
    --region europe-west9 \
    --platform managed \
    --service-account clourun-olenbee@olb-backoffice-api-uat.iam.gserviceaccount.com \
    --vpc-connector projects/olb-backoffice-api-uat/locations/europe-west9/connectors/olb-uat-network-glb-netspoke \
    --vpc-egress all-traffic \
    --port 8080 \
    --allow-unauthenticated


Backoffice Front UAT:
.....................
This section provides steps for deploying the frontend interface of the Backoffice application in dev.


1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-backoffice-front-uat/backofficefront/backofficefront .
   - docker psuh europe-west9-docker.pkg.dev/olb-backoffice-front-uat/backofficefront/backofficefront:latest

2- Deploy the Cloud Run instance

gcloud run deploy backofficefront \
   --image europe-west9-docker.pkg.dev/olb-backoffice-front-uat/backofficefront/backofficefront@sha256:4f1dd64c14b1e780a581e5704c697bf4d18a09ebfcb39929346b3219b22881d6 \
   --region europe-west9 \
   --platform managed \
   --service-account clourun-olenbee@olb-backoffice-front-uat.iam.gserviceaccount.com \
   --vpc-connector projects/olb-backoffice-api-uat/locations/europe-west9/connectors/olb-uat-network-glb-netspoke \
   --vpc-egress all-traffic \
   --port 8080 \
   --allow-unauthenticated
...
Beneficiaries API UAT:
......................
This section describes the process for deploying the Beneficiaries API in a dev environment.

1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-beneficiaries-api-uat/ms-appdata/ms-appdata .
   - docker psuh europe-west9-docker.pkg.dev/olb-beneficiaries-api-uat/ms-appdata/ms-appdata:latest

2- Deploy the Cloud Run instance

gcloud run deploy ms-appdata \
   --image europe-west9-docker.pkg.dev/olb-beneficiaries-api-uat/ms-appdata/ms-appdata@sha256:0e88c194d2bf686afa2528a073cbe83c5571ed2676d6e0c8ece8258cafe3a872 \
   --region europe-west9 \
   --platform managed \
   --service-account clourun-olenbee@olb-beneficiaries-api-uat.iam.gserviceaccount.com \
   --vpc-connector projects/olb-clientsapp-api-uat/locations/europe-west9/connectors/olb-uat-network-glb-netspoke \
   --vpc-egress all-traffic \
   --port 8080 \
   --allow-unauthenticated
