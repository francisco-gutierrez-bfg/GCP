...
At the top level, the Olenbee.com organization acts as the root entity, encapsulating all projects and resources under a unified management structure.
 ...
 PRD (Production Applications)
 .............................
 Houses all production workloads, ensuring a clear distinction between infrastructure and application projects.

 Contains critical application components:
  - Beneficiaries-API (e.g., olb-beneficiaries-api-prd-6530)
...
...........................................
 ...
 - Production (PRD) – Hosts production applications, including backend and frontend services.
  ...
  └── STANDARD <-- Parent Folder
      ...
      |
      ├── Network  ---> Network configurations for App Projects (PRD - PPRD - DEV - PPRD - QA - UAT)
      |   |_ Note: All projects use a shared VPC comming from olb-nethub-<env>, being <env> each environment (prd, pprd, dev, uat, qa)
      |   |        The app projects need to be related inside VPC --> Shared VPC section in order to be visible and usable on the destination app project.
      |   |
      │   ├── olb-nethub-prd  (olb-nethub-prd-e7ce)  <--- Contains DNS Zones to be used by all projects
      │   ├── olb-netspoke-dev (olb-netspoke-dev-6005)  <--- Contains shared VPC's to be used by dev projects 
      │   ├── olb-netspoke-pprd (olb-netspoke-pprd-b534)  <--- Contains shared VPC's to be used by pprd projects 
      │   ├── olb-netspoke-prd (olb-netspoke-prd-39fa)  <--- Contains shared VPC's to be used by prod projects
      │   ├── olb-netspoke-qa (olb-netspoke-qa-4b74)  <--- Contains shared VPC's to be used by qa projects
      │   └── olb-netspoke-uat (olb-netspoke-uat-8f33)  <--- Contains shared VPC's to be used by qa projects 
      |
      ...
      └── PRD  ---> Production APPS
          ...
          ├── Beneficiaries-API 
          |   | 
          │   └── olb-beneficiaries-api-prd-6530  
          |
          ├── Clients-WEBAPP
          |   |  
          │   ├── Backend  
          │   │   └── olb-clientsapp-api-prod  
          │   └── Frontend  
          │       └── olb-clientsapp-front-prod  <---- HTPS Load Balancer configured here pointing to DNS name "entreprise.olenbee.com" Reserved IP: 34.36.221.178
          | 
          └── Olenbee-BO  
              |
              ├── Backend  
              │   └── olb-backoffice-api-prod  
              └── Frontend  
                  └── olb-backoffice-front-prod  <---- HTTPS Load Balancer configured here pointing to DNS name "olb.olenbee.com"  Reserved IP: 34.95.105.33
...
ClientsApp API Prod:
....................
This section provides instructions for building and deploying the ClientsApp API in a production environment.

1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-clientsapp-api-prd/clientsapp/clientsapp .
   - docker psuh europe-west9-docker.pkg.dev/olb-clientsapp-api-prd/clientsapp/clientsapp:latest

2- Deploy the Cloud Run instance

gcloud run deploy clienstapp \
    --image europe-west9-docker.pkg.dev/olb-clientsapp-api-prd/clientsapp/clientsapp:latest \
    --region europe-west9 \
    --platform managed \
    --service-account clourun-olenbee@olb-clientsapp-api-prd.iam.gserviceaccount.com \
    --vpc-connector projects/olb-clientsapp-api-prd/locations/europe-west9/connectors/olb-prd-network-glb-netspoke \
    --vpc-egress all-traffic \
    --port 8080 \
    --allow-unauthenticated
...
ClientsApp Front Prod:
......................
This section outlines the steps to build and deploy the frontend application for ClientsApp in a production environment.

1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-clientsapp-front-prod/clientsapp-front/clientsapp-front .
   - docker psuh europe-west9-docker.pkg.dev/olb-clientsapp-front-prod/clientsapp-front/clientsapp-front:latest

2- Deploy the Cloud Run instance

gcloud run deploy clientsapp-front \
   --image europe-west9-docker.pkg.dev/olb-clientsapp-front-prod/clientsapp-front/clientsapp-front:latest \
   --region europe-west9 \
   --platform managed \
   --service-account clourun-olenbee@olb-clientsapp-front-prod.iam.gserviceaccount.com \
   --vpc-connector projects/olb-clientsapp-api-prod/locations/europe-west9/connectors/olb-prd-network-glb-netspoke \
   --vpc-egress all-traffic \
   --port 8080 \
   --allow-unauthenticated
...
1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-backoffice-api-prod/backoffice/backoffice .
   - docker psuh europe-west9-docker.pkg.dev/olb-backoffice-api-prod/backoffice/backoffice:latest

2- Deploy the Cloud Run instance

gcloud run deploy backoffice \
    --image europe-west9-docker.pkg.dev/olb-backoffice-api-prod/backoffice/backoffice:latest \
    --region europe-west9 \
    --platform managed \
...
Backoffice Front Prod:
......................
This section provides steps for deploying the frontend interface of the Backoffice application in production.


1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-backoffice-front-prod/backofficefront/backofficefront .
   - docker psuh europe-west9-docker.pkg.dev/olb-backoffice-front-prod/backofficefront/backofficefront:latest

2- Deploy the Cloud Run instance

gcloud run deploy backofficefront \
   --image europe-west9-docker.pkg.dev/olb-backoffice-front-prod/backofficefront/backofficefront@sha256:4f1dd64c14b1e780a581e5704c697bf4d18a09ebfcb39929346b3219b22881d6 \
   --region europe-west9 \
   --platform managed \
   --service-account clourun-olenbee@olb-backoffice-front-prod.iam.gserviceaccount.com \
   --vpc-connector projects/olb-backoffice-api-prod/locations/europe-west9/connectors/olb-prd-network-glb-netspoke \
   --vpc-egress all-traffic \
   --port 8080 \
   --allow-unauthenticated
...
Beneficiaries API Prod:
.......................
This section describes the process for deploying the Beneficiaries API in a production environment.

1- Build your image from the Dockerfile
   - docker build -t europe-west9-docker.pkg.dev/olb-beneficiaries-api-prod/ms-appdata/ms-appdata .
   - docker psuh europe-west9-docker.pkg.dev/olb-beneficiaries-api-prod/ms-appdata/ms-appdata:latest

2- Deploy the Cloud Run instance

gcloud run deploy ms-appdata \
   --image europe-west9-docker.pkg.dev/olb-beneficiaries-api-prod/ms-appdata/ms-appdata@sha256:0e88c194d2bf686afa2528a073cbe83c5571ed2676d6e0c8ece8258cafe3a872 \
   --region europe-west9 \
   --platform managed \
   --service-account clourun-olenbee@olb-beneficiaries-api-prod.iam.gserviceaccount.com \
   --vpc-connector projects/olb-clientsapp-api-prod/locations/europe-west9/connectors/olb-prd-network-glb-netspoke \
   --vpc-egress all-traffic \
   --port 8080 \
   --allow-unauthenticated
